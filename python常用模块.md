...
[os模块](https://so.csdn.net/so/search?q=os模块&spm=1001.2101.3001.7020)是Python中整理文件和目录最为常用的模块，该模块提供了非常丰富的方法用来处理文件和目录。
...
sys 模块主要负责与 Python [解释器](https://so.csdn.net/so/search?q=解释器&spm=1001.2101.3001.7020)进行交互，该模块提供了一系列用于控制 Python 运行环境的函数和变量。

```
sys.argv 命令行参数List，第一个元素是 运行的程序 本身的路径 
  args = sys.argv
  print(args)  
  ---------------
...
```
dump:传入一个python对象 序列化，写入到文件
dumps:传入一个python对象 序列化，返回对象
load:反序列化，从文件
loads:反序列化，从对象
```
...
```
dump:传入一个python对象 将其编码为json格式后存储文件
dumps:传入一个python对象 编码为json格式后存储到str中
load:传入一个json格式的文件流，将其解码为python对象
loads:传入一个json格式的str，将其解码为python对象
```

### 10.shutil模块

作为os模块的补充，提供了复制、移动、删除、压缩、解压等操作，这些 os 模块中一般是没有提供的。

```
copy(src,dst):复制文件
copytree(src,dst): 复制文件夹 这里只能是移动到一个空文件夹
move(src,dst):移动文件/文件夹
.rmtree(src):彻底删除文件
```
